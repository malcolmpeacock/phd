...
# calculate peak demand and ramp rate for just the electric heat.
gas_diff = gas.diff()
bdew_diff = bdew.diff()
...
electric_daily = electric.resample('D').sum()
gas_daily = gas.resample('D').sum()
bdew_daily = bdew.resample('D').sum()
watson_daily = watson.resample('D').sum()
hdd155_daily = hdd155.resample('D').sum()
hdd128_daily = hdd128.resample('D').sum()

# output plots

gas_daily.plot(label='Electric Heat Gas', fontsize=15)
bdew_daily.plot(label='Electric Heat BDEW', fontsize=15)
watson_daily.plot(label='Electric Heat Watson', fontsize=15)
hdd155_daily.plot(label='Electric Heat HDD15.5', fontsize=15)
hdd128_daily.plot(label='Electric Heat HDD12.8', fontsize=15)
electric_daily.plot(label='Historic Electricity', fontsize=15)
...
print('Method Demand Max  Min     Sum      Ramp Rate Max Min')
print('Gas      {:.2f}    {:.2f}  {:.2f}       {:.2f}   {:.2f}'.format(gas.max(), gas.min(), gas.sum(), gas_diff.max(), gas_diff.min() ) )
print('BDEW     {:.2f}    {:.2f}  {:.2f}       {:.2f}   {:.2f}'.format(bdew.max(), bdew.min(), bdew.sum(), bdew_diff.max(), bdew_diff.min() ) )
print('Watson   {:.2f}    {:.2f}  {:.2f}       {:.2f}   {:.2f}'.format(watson.max(), watson.min(), watson.sum(), watson_diff.max(), watson_diff.min() ) )
print('HDD155   {:.2f}    {:.2f}  {:.2f}       {:.2f}   {:.2f}'.format(hdd155.max(), hdd155.min(), hdd155.sum(), hdd155_diff.max(), hdd155_diff.min() ) )
print('HDD128   {:.2f}    {:.2f}  {:.2f}       {:.2f}   {:.2f}'.format(hdd128.max(), hdd128.min(), hdd128.sum(), hdd128_diff.max(), hdd128_diff.min() ) )
print('historic {:.2f}    {:.2f}  {:.2f}       {:.2f}   {:.2f}'.format(electric.max(), electric.min(), electric.sum(), electric_diff.max(), electric_diff.min() ) )
...
gas_daily.plot(label='Electric with Heat Gas', fontsize=15)
bdew_daily.plot(label='Electric with Heat BDEW', fontsize=15)
watson_daily.plot(label='Electric with Heat Watson', fontsize=15)
hdd155_daily.plot(label='Electric with Heat HDD15.5', fontsize=15)
hdd128_daily.plot(label='Electric with Heat HDD12.8', fontsize=15)
electric_daily.plot(label='Historic Electricity', fontsize=15)
plt.title('Electrification of heat 2018')
plt.xlabel('Day of the year', fontsize=15)
plt.ylabel('Daily Demand (TWh)', fontsize=15)
