...
def storage(net_demand, eta=0.75, etad=0.75, hydrogen=None):
...
def storage_mp(net_demand, eta=0.75, etad=0.75, hydrogen=None):
    history = pd.Series(np.zeros(len(net_demand)), index=net_demand.index, name='store')
    store = 0.0
    eta_charge = eta
    eta_discharge = etad
...
def storage_grid_config(demand, wind, pv, eta, etad, hourly, base, variable, hydrogen, method, f_wind, f_pv, threshold, constraints, store_max, debug, results):
    ...
    if method == 'kf':
        store_hist = storage(net, eta, etad, hydrogen) * -1.0
        store_size = store_hist.max()
    else:
        if method == 'mp':
            store_hist = storage_mp(net, eta, etad, hydrogen) * -1.0
            store_size = store_hist.max() - store_hist.min()
        else:
            store_size, store_hist, variable_total = storage_all(demand, wind, pv, base, variable, eta, etad, hydrogen, f_wind, f_pv, threshold, constraints, store_max / store_factor, debug)
...
def storage_cardenas(demand, wind, pv, eta, etad, hourly=False, npv=14, nwind=14, step=0.5, base=0.0, hydrogen=None, constraints='new', hist_wind=1.0, hist_pv=1.0, hist_days=30, threshold=0.01, variable=0.0, contours='med', debug=False):
    ...
    for omega in [0.05, 0.1, 0.15, 0.20, 0.25]:
        for Xw in np.arange(0, 1, 0.1):
            ...
            while check<1:
                ...
                DnetPos = Dnet.clip(lower= 0.0)
                Epos = DnetPos.sum()
                Elost = (Epos / eta ) - Epos
                Eomega = (Ed * Xr) * omega
                check = ( (Wmod.sum() + Smod.sum() ) - Elost - Eomega ) / Ed * Xr
                Eneed = Eneed*1.1
                print(" omega {} Xw {} Xs {} Ew {} Es {} Epos {} Elost {} Eomega {} check {} Eneed {} Ed {} ".format( omega, Xw , Xs , Ew , Es , Epos , Elost , Eomega , check , Eneed, Ed) )
            print('Profile of net demand created!')
            # modify net demand to account for efficiency
            DnetNeg = Dnet.clip(upper = 0.0)
            ...
            Klast = 0
            KlastDelta = 0.01
            while K>omega and abs(K-Klast)>KlastDelta:
#               SoC = 0
                S = []
                excess = 0
                store_size += 0.1
                for index, net_value in DnetMod.items():
                    value = net_value / store_size
                    # Net demand is negative - Add energy to the store
                    if value <0:
                        add_amount = min(-value, (1 - SoC) )
                        SoC = SoC + add_amount
                        excess -= ( add_amount + value ) * store_size
                    else:
                    # Otherwise, take energy from the store
                        # Fossil fuels
                        if value > SoC:
...
#                           break;
                    ...
                    if SoC<0.0 or SoC>1.0:
                        print()
                        print('ERROR: Soc outside range')
                        print('value {} SoC {} add_amount {} K {} store_size {} '.format(value, SoC, add_amount, K, store_size) )
                        quit()
                energy = Wmod.sum() + Smod.sum()
#               energy = Wmod.sum() + Smod.sum() + dispatchable
                Klast = K
#               K = excess / energy
                K = excess / Ed
...
#               print('K {} omega {} store_size {} dispatchable {}'.format(K, omega, store_size, dispatchable) )
            ...
            print(' ')
            if abs(K - Klast) <= KlastDelta:
                print(' K stopped changing - no solution found omega {} K {}'.format( omega, K ) )
            else:
                ...
                results['f_pv'].append(f_pv)
                results['f_wind'].append(f_wind)
                results['variable_energy'].append(dispatchable)
                # store at the start of the year
                yearly_store = store_hist.resample('Y').first()
                year_store_min = yearly_store.min() / store_size
                year_store_max = yearly_store.max() / store_size
...
def storage_grid_new(demand, wind, pv, eta, etad, hourly=False, npv=14, nwind=14, step=0.5, base=0.0, hydrogen=None, constraints='new', hist_wind=1.0, hist_pv=1.0, hist_days=30, threshold=0.01, variable=0.0, contours='med', debug=False):
    ...
    balanced, sample_hist, variable_total = storage_balance(demand, wind, pv, eta, etad, base, hydrogen, hist_pv, hist_wind, hist_days, constraints, variable, debug)
...
#           wind_min = max( 1 - f_pv, 0.0 )
            wind_min = 0.0
            f_wind = wind_max
            balanced, store_hist, variable_total = storage_balance(demand, wind, pv, eta, etad, base, hydrogen, f_pv, f_wind, store_size, constraints, variable)
            if not balanced:
...
def storage_balance(demand, wind, pv, eta, etad, base, hydrogen, f_pv, f_wind, capacity, constraints='new', variable=0.0, debug=False):
...
def storage_all(demand, wind, pv, base, variable, eta, etad, hydrogen, f_wind, f_pv, threshold=0.01, constraints = 'new', store_max = 60, debug=False):
    # try different amounts of storage 
    store_min = 0.0
    store_size = store_max
    balanced, store_hist, variable_total = storage_balance(demand, wind, pv, eta, etad, base, hydrogen, f_pv, f_wind, store_size, constraints, variable, debug)
    if not balanced:
        ...
        while abs(store_max - store_min) > threshold:
            ...
            balanced, last_hist, variable_total = storage_balance(demand, wind, pv, eta, etad, base, hydrogen, f_pv, f_wind, store_size, constraints, variable, debug)
